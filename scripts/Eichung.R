...
#drawCI(x, Mo, sqrt(Mo), scol="black")
#fit3=gausfit(data.frame(x=x,y=Mo, sy=sqrt(Mo)),c(135,300), weighted=TRUE)
#plotgaus(fit3,c(135,290),col="red")

#drawCI(x, Cu, sqrt(Cu), scol="black")

#drawCI(x, Ag, sqrt(Ag), scol="black")
#fit2=gausfit(data.frame(x=x,y=Ag, sy=sqrt(Ag)),c(150,430), weighted=TRUE)
#plotgaus(fit2,c(150,430),col="red")

#drawCI(x, Ba, sqrt(Ba), scol="black")
